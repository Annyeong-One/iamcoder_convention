2024년 8월 7일 이후로 나는코더다에서 주최하거나 출제하는 문제는 다음의 포매팅을 따라 출제합니다.
...
- ‘N개’와 같은 단어가 언급된 경우 ‘한 개’ 대신 ‘1개’를 사용합니다.
...
- 변수의 제한에서 '자연수'라는 단어는 사용하지 않습니다. 문맥에 따라 '양의 정수' 혹은 '음이 아닌 정수'로 바꿔 사용합니다.
	- 굳이 양의 정수 혹은 음이 아닌 정수라는 사실을 언급할 필요가 없는 문제면 언급하지 않음을 권장합니다.
...
- 수학 기호들은 문제와 참가자 수준에 맞도록 사용하는 것을 권장합니다. 
	- 디스크립션이 너무 길고, 수학 기호를 사용함으로서 디스크립션을 훨씬 더 간결하게 만들 수 있다면, 힌트 섹션에 해당 기호에 대한 정의를 제공합니다.
...
- 변수의 범위 제한은 [제한] 섹션에 적습니다. 필요한 경우 [입력] 섹션에서 해당 상수나 변수가 처음 등장하는 위치에도 적어도 됩니다. 문제 본문에는 적지 않습니다.
	- 변수에 인덱스가 들어갈 경우 본문에 인덱스의 범위를 명시해도 됩니다.
- 범위 제한을 여러 개 적을 경우에는 세미콜론(;)으로 구분합니다.
- 네자리 수 혹은 그 이하를 쓸 때는 붙여 적고, 다섯자리에서 일곱자리 수를 표기할 때는 세 자리씩 끊어서 적습니다.
	- 예로, $3141$은 $3141$, $31415$는 $31\,415$, 314159는 $314\,159$, $3141592$은 $3\,141\,592$으로 표기합니다.
- 여덟자리 수 (10 000 000) 이상을 표기하는 경우 지수를 이용해 표기할 수 있으면 그렇게 표기합니다. 불가능하다면, 세 자리씩 끊어서 적습니다.
	- $1000000000$은 $10^9$로, $2 000 000 000$은 $2 \times 10^9$로 표기합니다.
	- $998244353$은 $998\,244\,353$으로 표기합니다.
- 만약 어떤 수가 특별한 의미를 가진 경우, 그 의미가 문제에서 중요한 역할을 하는 경우,
